...
ifndef DEBUG
	CFLAGS += -O2 -funwind-tables -Wl,--export-dynamic
else
	#CFLAGS += -g -rdynamic -funwind-tables -DDEBUG -Wl,--export-dynamic
	CFLAGS += -O2 -ggdb -funwind-tables -DDEBUG -Wl,--export-dynamic
	#CFLAGS += -O2
endif
...
# AROS
ifeq ($(PLATFORM),aros)
	CPPFLAGS += --sysroot=$$HOME/arosbuilds/core-linux-x86_64-d/bin/linux-x86_64/AROS/Development -DCPU_x86_64=1 -D_FILE_OFFSET_BITS=64 -UUSE_LIBSERIALPORT -UUSE_OPENGL -I$$HOME/arosbuilds/core-linux-x86_64-d/bin/linux-x86_64/AROS/Development/include
	CFLAGS += -fomit-frame-pointer -Wno-unused -Wno-format -fno-strict-aliasing
	SDL_CFLAGS := -I$$HOME/arosbuilds/core-linux-x86_64-d/bin/linux-x86_64/AROS/Development/include/SDL2 -I$$HOME/arosbuilds/amiberry_3rd/3rd/include/
	SDL_LDFLAGS := -lSDL2
	LDFLAGS = $(SDL_LDFLAGS) -lSDL2_image -lSDL2_ttf -lguisan -Lexternal/libguisan/lib -lmt32emu -Lexternal/mt32emu
	LDFLAGS += -L$$HOME/arosbuilds/core-linux-x86_64-d/bin/linux-x86_64/AROS/Development/lib -L$$HOME/arosbuilds/amiberry_3rd/3rd/lib -Wl,-O1 -Wl,--hash-style=gnu -Wl,--as-needed -lpthread -lz -lpng -lFLAC -lmpg123 -lmpeg2 -lmpeg2convert -lguichan -lguichan_sdl -lGL -liconv -logg -lnet -no-pie
	AS     = x86_64-pc-aros-as
	CC     = x86_64-pc-aros-gcc
	CXX    = x86_64-pc-aros-g++
	STRIP  ?= x86_64-pc-aros-strip
	ifdef DEBUG
		LDFLAGS += -lstdc++
	endif

# Raspberry Pi 2/3/4/5 (SDL2)
else ifeq ($(PLATFORM),$(filter $(PLATFORM),rpi2-sdl2 rpi3-sdl2 rpi4-sdl2 rpi5-sdl2))
	CPPFLAGS += $(CPPFLAGS32)
	CPPFLAGS += $(NEON_FLAGS)
	HAVE_NEON = 1
...
export CFLAGS := $(CPUFLAGS) $(CFLAGS) $(EXTRA_CFLAGS) -DAMIBERRY_DATADIR=\"PROGDIR\" -DAMIBERRY_LIBDIR=\"PROGDIR\"
export CXXFLAGS = $(CFLAGS) -std=gnu++17 -DAMIBERRY_DATADIR=\"PROGDIR\" -DAMIBERRY_LIBDIR=\"PROGDIR\" 
export CPPFLAGS
...
src/cpuemu%.o: src/cpuemu%.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -O0 -c $< -o $@

DEPS = $(OBJS:%.o=%.d) $(C_OBJS:%.o=%.d)

# disabled
# floppybridge capsimg
#
$(PROG): $(OBJS) $(C_OBJS) guisan mt32emu capsimg
...
#	$(RM) external/floppybridge/libfloppybridge.so
	...
	$(RM) external/capsimage/libcapsimage.a
...
mt32emu:
	cmake -DCMAKE_BUILD_TYPE=Debug -Dlibmt32emu_SHARED=FALSE -S external/mt32emu -B external/mt32emu/build
	cmake --build external/mt32emu/build --target all --parallel --verbose
...
capsimg:
	...
	cp external/capsimage/build/libcapsimage.a ./plugins/
