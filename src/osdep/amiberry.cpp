...
static int sleep_millis2(int ms, const bool main)
{
	...
	if (main) {
#ifndef __AROS__ /* SDL_CondWaitTimeout causes SDL_UnlockMutex to being called 
                    on a mutex that is not locked, but our implementation using 
					Semaphore don't allow that and this will crash AROS */ 
